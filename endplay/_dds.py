...
ctypes wrapper around the C++ library, and a copy of the dds library file
(usually dds.dll or dds.so) must be in your PATH environment or the `init`
function explicitly called with the path to where it is located on your machine.
The only extra convenience provided is to wrap all of the dds functions so that
instead of returning error codes they raise a `DDSError`.
"""
