...
def builder_inited(app):
	...
	for section in app.config.index_sections:
		...
		if path.is_file():
			sections.append(str(path.with_suffix("").relative_to(pages_dir)))
		else:
			subsections = [str(file.with_suffix("").relative_to(pages_dir)) for file in path.iterdir()]
			sections += sorted(subsections)

	# tee input to output, replacing the %SECTIONS% tag with the actual sections
	infile, outfile = app.config.index_template_file, Path(pages_dir, "index.rst")
