...
class Deal:
	...
	* The cards played to the current trick in `Deal.curtrick`
    ...
    def __init__(self, pbn: Optional[str] = None, first: Player = Player.north, trump: Denom = Denom.nt, *, complete_deal: bool = False):
    ...
    def to_json(self, indent: Optional[int] = None) -> str:
        ...
        d = {"north": self.north.to_pbn(), "east": self.east.to_pbn(), "south": self.south.to_pbn(), "west": self.west.to_pbn(), "curtrick": [str(card) for card in self.curtrick], "first": self.first.name, "trump": self.trump.name}
    ...
    def pprint(self, board_no: Optional[int] = None, exclude: list[Player] = [], stream=sys.stdout, *, vul: Optional[Vul] = None, dealer: Optional[Player] = None) -> None:
        """
