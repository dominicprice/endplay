...
def main():
    ...
    parser.add_argument("-g", type=int, default=1000000, help="Maximum number of hands to generate (default 1000000).")
    deprecated.add_argument("-l", type=int, help="Instead of shuffling, deals are read from the file library.dat by M. Ginsberg")
    ...
    deprecated.add_argument("-q", action="store_true", help="Suppress PBN output (useful for testing, then switch it back on when generating the \"final\" sample).")
    parser.add_argument("-s", type=int, help="Uses number as the seed for the random generator, running the program twice with the same seed, will produce the same sequence of hands.")
    parser.add_argument("-u", action="store_true", help="Use unicode card symbols instead of the letters S, H, D, C")
    parser.add_argument("-v", action="count", default=0, help="Verbose output, can be specified up to three times (-vvv). Level 1 prints statistics at the end of the run, level 2 prints any runtime generated warnings and level 3 prints full stacktraces if an exception occurs.")
    ...
    swapping.add_argument("-0", action="store_const", dest="swapping", const=0, help="No swapping, each deal is generated normally (shuffling), default.")
    swapping.add_argument("-2", action="store_const", dest="swapping", const=2, help="2-way swapping, after each shuffle another deal is generated by permuting E and W, leaving N and S in place (NB: not fully compatible with predeal).")
    swapping.add_argument("-3", action="store_const", dest="swapping", const=3, help="3-way swapping, after each shuffle another 5 deals are generated by permuting E, W, and S every which way, leaving N in place (NB: not fully compatible with predeal). not needed (NB: not compatible at all with predeal).")
    parser.add_argument("-c", action='append', default=[], help="Constraint to be applied, can be specified more than once for multiple constraints")
    ...
    parser.add_argument("-a", action='append', default=[], help="Action to be applied, can be specified more than once")
    ...
    # And away we go
    try:
        run_script(args.script, show_progress=args.m, produce=args.p, generate=args.g, seed=args.s, verbose=args.v > 0, swapping=args.swapping, outformat=args.f, outfile=args.o, constraints=args.c, actions=args.a, predeal=args.d, board_numbers=args.b)
