...
class PDFActionsWriter(LaTeXActionsWriter):
    ...
    def __exit__(self, *_):
        ...
        proc = run(["pdflatex", "main.tex"], cwd=self.tmpdir, input=b'', capture_output=True)
        if proc.returncode != 0:
            with open(self.tmpdir + "/main.log") as f:
                log = f.read()
            with open(self.tmpdir + "/main.tex") as f:
                tex = f.read()
            raise RuntimeError("error running pdflatex:main.log:\n" + log + "\ninput file:\n" + tex)
        if self.fname is None:
            raise RuntimeError("output file must be specified for pdf actions")
        shutil.copy2(os.path.join(self.tmpdir, "main.pdf").encode(), self.fname.encode())
        try:
