...
# update config.py
sed -i "s/__version__ = .*/__version__ = \"${ENDPLAY_VERSION}\"/g" 'src/endplay/config.py'
sed -i "s/__version_info__ = .*/__version__ = (${PARTS[0]}, ${PARTS[1]}, ${PARTS[2]})/g" 'src/endplay/config.py'
